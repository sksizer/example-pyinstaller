    ...
    try:
        ...
        if test_dir.exists() and test_dir.is_dir():
            results.append(
                TestResult(
                    ...
                    message=f"Created directory at {test_dir}"
                )
            )
        else:
            results.append(
                TestResult(
                    ...
                    message=f"Failed to create directory at {test_dir}"
                )
            )
        ...
        if test_file.exists():
            results.append(
                TestResult(
                    status=TestStatus.SUCCESS,
                    message=f"Created file at {test_file}"
                )
            )
        ...
        if content == test_content:
            results.append(
                TestResult(
                    status=TestStatus.SUCCESS,
                    message=f"File content verified: '{content}'"
                )
            )
        else:
            results.append(
        ...
        if not test_dir.exists():
            ...
            results.append(
                TestResult(
                    ...
                    message="Cleanup incomplete - directory still exists"
                )
            )
        ...
        results.append(
            TestResult(
                status=TestStatus.FAILURE,
                message=f"User directory operation failed: {str(e)}"
            )
        )
        # Attempt cleanup
        try:
